...
class PositionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (period_end - period_start).days > 365:
            self._errors["period_end"] = self.error_class(
                [_("Du kan kun registrer verv i ett år av gangen.")]
            )

        # Checks is period_start is a later date than period_end.
        if period_start > period_end:
            self._errors["period_start"] = self.error_class(
                [_("Start-dato kan ikke være etter endt-dato.")]
            )
...
class MembershipSettingsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"started_date": forms.TextInput(attrs={"placeholder": "YYYY-MM-DD"})}
    ...
    def clean(self):
        ...
        if self.is_valid():
            ...
            user = auth.authenticate(
                ...
                password=cleaned_data["ow4_password"],
            )
